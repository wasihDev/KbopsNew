...
import 'package:kbops/vote_screens/widgets/expension_webView.dart';
import 'package:kbops/vote_screens/widgets/participant_Card.dart';
...
class _VoteNowState extends State<VoteNow> {
  ...
  void initState() {
    ...
    log("INIT CALL AGAUB");
    WidgetsBinding.instance.addPostFrameCallback((_) async {
      Provider.of<VoteNowProvider>(context, listen: false)
          .getEventInfo(eventsInfo.EventID!);

      Provider.of<VoteNowProvider>(context, listen: false)
          .getParticipants(eventsInfo.EventID!);
  ...
  Widget build(BuildContext context) {
    // print(eventsInfo.EventTotalVotes);
    log('==========>>>> BUILD WIDGET');
    // final getParticenpent = Provider.of<VoteNowProvider>(context);
    ...
    return Scaffold(
      backgroundColor: Colors.white,
      body: Consumer<VoteNowProvider>(builder: (context, votingProvider, _) {
        return SafeArea(
          child: Stack(
            children: [
              SingleChildScrollView(
                child: Column(
                  ...
                  children: [
                    ...
                    ExpensionWebView(eventsInfo: eventsInfo),
                    ListView.builder(
                        shrinkWrap: true,
                        physics: const ClampingScrollPhysics(),
                        itemCount: votingProvider.participantsList.length,
                        itemBuilder: (BuildContext context, int index) {
                          final participant =
                              votingProvider.participantsList[index];
                          ...
                          double percent = (participant.ParticipantTotalVotes! /
                                      eventsInfo.EventTotalVotes!)
                                  .clamp(0.0, 1.0) *
                              100;
                          ...
                          return ParticipantCard(
                            ...
                            ontap: () async {
                              log('message');
                              // await getUserInfo();
                              Provider.of<VoteNowProvider>(context,
                                      listen: false)
                                  .toggleValue();
                    ...
                    //             //     .getUserData();
                    //             // log('userinfoList ${userinfo.nameInfoList}');
                    //             log('eventsInfo ${widget.eventsInfo}');
                    //             log('userInfo ${userinfo.userInfo}');
                    //             Navigator.push(
                    //                 context,
                    //                 MaterialPageRoute(
                    //                     builder: (_) => AllComments(
              ...
              Provider.of<VoteNowProvider>(context, listen: false).state
                  ? Container(
                      height: MediaQuery.of(context).size.height * 1,
                      width: MediaQuery.of(context).size.width * 1,
                      color: Colors.black54,
                                  ...
                                  child: Row(
                                    children: [
                                      ...
                                      Consumer<UserProvider>(
                                          builder: (context, userinfo, child) {
                                        return Text(
                                          '${userinfo.userInfo?.TotalKPoints ?? ''}',
                                          style: const TextStyle(
                                              fontSize: 16,
                                              fontWeight: FontWeight.bold,
                                              color: Colors.white),
                                                  ...
                                                  onPressed: () async {
                                                    final userinfo = Provider
                                                        .of<UserProvider>(
                                                            context,
                                                            listen: false);
                                                    log('vote: ${pointsController.text}');
                                                    log('EvendId: ${eventsInfo.EventID!}');
                                                    log('participantId: $participantId');
                                                    log('UserId: ${userinfo.userInfo?.UserID}');
                                                    hideKeyboard(context);
                                                    if (_formkey.currentState!
                                                            .validate() &&
                                                        userinfo.userInfo !=
